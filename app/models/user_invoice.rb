class UserInvoice < ActiveRecord::Base
  ...
  has_many :executor_task_to_user_invoices, ->{TaskToUserInvoice.executor}, :class_name => 'TaskToUserInvoice', :inverse_of => :user_invoice
  has_many :account_manager_task_to_user_invoices, ->{TaskToUserInvoice.account_manager}, :class_name => 'TaskToUserInvoice', :inverse_of => :user_invoice
  has_many :team_lead_task_to_user_invoices, ->{TaskToUserInvoice.team_lead}, :class_name => 'TaskToUserInvoice', :inverse_of => :user_invoice

  has_many :executor_tasks, :class_name=>'Task', :through => :executor_task_to_user_invoices, :source=>:task
  has_many :account_manager_tasks, :class_name=>'Task', :through => :account_manager_task_to_user_invoices, :source=>:task
  has_many :team_lead_tasks, :class_name=>'Task', :through => :team_lead_task_to_user_invoices, :source=>:task
  validates :user, :organization, :presence => true
